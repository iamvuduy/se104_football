...
const { generateGoalCode } = require("../services/idGeneratorService");
...
router.post("/", admin, async (req, res) => {
  ...
  if (scoreParts.length !== 2 || isNaN(scoreParts[0]) || isNaN(scoreParts[1])) {
    return res.status(400).json({
      error: "Tỷ số không hợp lệ. Vui lòng nhập theo định dạng 'X-Y'.",
    });
  }
  ...
  const team1GoalCount = goals.filter(
    (g) => String(g.team) === team1Str
  ).length;
  const team2GoalCount = goals.filter(
    (g) => String(g.team) === team2Str
  ).length;
  ...
  for (const goal of goals) {
    ...
    if (!goal.player || goal.player === "") {
      ...
      return res.status(400).json({
        error: `Thiếu thông tin cầu thủ ghi bàn. Vui lòng chọn cầu thủ cho tất cả các bàn thắng.`,
      });
    }
    if (!goal.team || goal.team === "") {
      console.log("VALIDATION FAILED: Missing team");
      return res.status(400).json({
        error: `Thiếu thông tin đội ghi bàn. Vui lòng chọn đội cho tất cả các bàn thắng.`,
      });
    }
    if (!goal.type || goal.type === "") {
      console.log("VALIDATION FAILED: Missing goal type");
      return res.status(400).json({
        error: `Thiếu loại bàn thắng. Vui lòng chọn loại cho tất cả các bàn thắng.`,
      });
    }
    if (goal.time === undefined || goal.time === null || goal.time === "") {
      console.log("VALIDATION FAILED: Missing time");
      return res.status(400).json({
        error: `Thiếu thời điểm ghi bàn. Vui lòng nhập thời điểm cho tất cả các bàn thắng.`,
      });
    }
    ...
    getMatchCode((match_code) => {
      ...
      db.run(
        ...
        function (err) {
          ...
          // Get the next goal code number to avoid duplicates
          db.get(
            ...
            (err, row) => {
              if (err) {
                ...
                return res
                  .status(500)
                  .json({ error: "Could not generate goal codes." });
              }

              let nextGoalNumber = (row?.max_number || 0) + 1;
              console.log("DEBUG: Starting goal code from:", nextGoalNumber);

              // If no goals, insert them sequentially
              if (goals.length === 0) {
                db.run("COMMIT", (err) => {
                  if (err) {
                    db.run("ROLLBACK");
                    ...
                    return res
                      .status(500)
                      .json({ error: "Could not commit transaction." });
                  }
              ...
              // Insert goals one by one sequentially to avoid race conditions
              let currentIndex = 0;
              const insertNextGoal = () => {
                if (currentIndex >= goals.length) {
                  // All goals inserted successfully
                  db.run("COMMIT", (err) => {
                    if (err) {
                      db.run("ROLLBACK");
                      console.error(
                ...
                if (
                  ...
                  db.run("ROLLBACK");
                  return res.status(400).json({
                    error: "Invalid goal data provided.",
                  });
                }

                const goalCode = `G${String(
                  nextGoalNumber + currentIndex
                ).padStart(3, "0")}`;
                console.log(
                  `DEBUG: Inserting goal ${currentIndex + 1} with code:`,
                  goalCode
                );
                ...
                db.run(
                  ...
                  function (err) {
                    ...
                    currentIndex++;
                    insertNextGoal(); // Insert next goal
                  }
                );
              };

              insertNextGoal(); // Start inserting goals
            }
          );
        }
      );
...
router.delete("/:id", admin, (req, res) => {
  ...
  db.run("BEGIN TRANSACTION", (err) => {
    ...
    db.run(
      "DELETE FROM goals WHERE match_result_id = ?",
      [matchId],
      function (err) {
        if (err) {
          db.run("ROLLBACK");
          ...
          return res
            .status(500)
            .json({ error: "Could not delete match goals." });
        }

        // Then delete the match result
        db.run(
          "DELETE FROM match_results WHERE id = ?",
          [matchId],
          function (err) {
            if (err) {
              db.run("ROLLBACK");
              ...
              return res
                .status(500)
                .json({ error: "Could not delete match result." });
            }
            ...
            db.run("COMMIT", (err) => {
              ...
              // Clear all leaderboard reports cache after deleting match result
              db.run("DELETE FROM leaderboard_reports", [], (err) => {
                if (err) {
                  console.error("Error clearing reports cache:", err.message);
                  // Don't return error, just log it - match was deleted successfully
                }
              });
...
router.put("/:id", admin, async (req, res) => {
  ...
  for (const goal of goals) {
    ...
    const goalMinute = Number(goal.time);
    if (
      !Number.isFinite(goalMinute) ||
      goalMinute < 0 ||
      goalMinute > goalTimeLimit
    ) {
  ...
  db.run("BEGIN TRANSACTION", (err) => {
    ...
    db.run(
      updateMatchSql,
      [team1, team2, score, stadium, date, time, matchId],
      function (err) {
        if (err) {
          db.run("ROLLBACK");
          ...
          return res
            .status(500)
            .json({ error: "Could not update match result." });
        }
        ...
        db.run(
          ...
          function (err) {
            if (err) {
              ...
              return res
                .status(500)
                .json({ error: "Could not delete old goals." });
            }
                ...
                Promise.all(insertGoalPromises)
                  .then(() => {
                    db.run("COMMIT", (err) => {
                      ...
                      // Clear all leaderboard reports cache after updating match result
                      db.run("DELETE FROM leaderboard_reports", [], (err) => {
                        if (err) {
                          console.error(
                            "Error clearing reports cache:",
                            err.message
                          );
                          // Don't return error, just log it - match was updated successfully
                        }
                      });
                  ...
                  .catch((error) => {
                    db.run("ROLLBACK");
                    return res.status(500).json({
                      error: "An error occurred while updating goals.",
                    });
                  });
              }
            );
